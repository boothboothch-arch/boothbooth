...
export default async function AdminDashboard({ searchParams }: { searchParams: Params }) {
  ...
  const cards = [
    ...
    { label: '입금 대기', value: `${rows.filter((order) => order.payment_state === 'pending' && order.order_state !== 'cancelled').length}건`, icon: Banknote },
    ...
    { label: '확인 필요', value: `${rows.filter((order) => order.payment_state === 'review_required').length}건`, icon: AlertCircle },
    { label: '입금 완료', value: `${rows.filter((order) => order.payment_state === 'paid').length}건`, icon: CheckCircle2 },
    { label: '배송·수령 준비', value: `${rows.filter((order) => order.order_state === 'preparing').length}건`, icon: Package },
    { label: '완료', value: `${rows.filter((order) => order.order_state === 'completed').length}건`, icon: PackageCheck },
  ...
  return (
    <>
      ...
      <section className="admin-panel"><div className="admin-panel__header"><h2>최근 주문</h2><Link href={{ pathname: '/admin/orders', query: selectedSale ? { saleId: selectedSale.id } : undefined }}>전체 보기</Link></div><div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>주문번호</th><th>주문자</th><th>금액</th><th>주문 상태</th><th>입금 상태</th><th>접수일</th></tr></thead><tbody>{rows.slice(0, 8).map((order) => <tr key={order.id}><td><Link href={`/admin/orders/${order.order_number}`}>{order.order_number}</Link></td><td>{order.customer_name}</td><td>{order.total_amount.toLocaleString('ko-KR')}원</td><td><Badge tone={order.order_state === 'cancelled' ? 'red' : 'blue'}>{orderStateLabel[order.order_state as OrderView['orderState']]}</Badge></td><td>{paymentStateLabel[order.payment_state as OrderView['paymentState']]}</td><td>{new Date(order.created_at).toLocaleDateString('ko-KR')}</td></tr>)}</tbody></table>{!rows.length && <div className="admin-empty">이 차수에 접수된 주문이 없습니다.</div>}</div></section>
    </>
  )
}
