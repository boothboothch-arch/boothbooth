    ...
    <>
      ...
      <div className="admin-detail-grid">
        ...
        <div className="admin-side-stack"><form className="admin-panel admin-state-form" action={updateOrderAction}>
          <input type="hidden" name="orderId" value={order.id} /><input type="hidden" name="orderNumber" value={order.orderNumber} /><input type="hidden" name="fulfillmentType" value={order.fulfillmentType} /><input type="hidden" name="previousPaymentState" value={order.paymentState} />{order.orderState === 'cancelled' && <input type="hidden" name="cancellationReason" value={order.cancellationReason ?? '관리자 취소'} />}
          <h2>상태 변경</h2><div className="field admin-state-form__status" data-state={order.orderState}><label>현재 주문 상태</label><select name="orderState" defaultValue={order.orderState}>{orderStateOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}{order.orderState === 'cancelled' && <option value="cancelled">취소</option>}</select><span className="field__hint">입금 상태는 선택한 주문 상태에 맞춰 자동으로 반영됩니다.</span></div>
