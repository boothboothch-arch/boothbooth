...
export default async function AdminOrderPage({ params, searchParams }: Props) {
  ...
  if (!order) notFound()
  const { data: changeLogs } = await createPrivilegedClient().from('order_item_change_logs').select('id,order_item_id,before_data,after_data,order_total_before,order_total_after,created_at').eq('order_id', order.id).order('created_at', { ascending: false })
