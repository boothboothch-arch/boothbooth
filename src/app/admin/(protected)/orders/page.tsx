...
import { orderStateLabel, paymentStateLabel, type OrderView } from '@/features/order/domain/order'
import { hmac, normalizeEmail, normalizePhone } from '@/server/security/crypto'

type Params = Promise<{ saleId?: string; q?: string; state?: string; payment?: string; from?: string; to?: string; itemType?: string; fulfillment?: string; overdue?: string; size?: string }>
...
export default async function AdminOrdersPage({ searchParams }: { searchParams: Params }) {
  ...
  const { q = '', state = '', payment = '', from = '', to = '', itemType = '', fulfillment = '', overdue = '', size = '' } = params
  ...
  if (saleId !== 'all') {
    const { data: saleProducts } = await client.from('products').select('id').eq('sale_id', saleId)
    productIds = (saleProducts ?? []).map((product) => product.id)
  }
  let optionQuery = client.from('product_options').select('product_id,option_type,value,sort_order').eq('active', true).order('sort_order')
  if (productIds) optionQuery = optionQuery.in('product_id', productIds.length ? productIds : ['00000000-0000-0000-0000-000000000000'])
  const { data: optionRows } = await optionQuery
  const sizes = [...new Set((optionRows ?? []).filter((item) => item.option_type === 'size').map((item) => item.value))]
  const csvParams = new URLSearchParams()
  csvParams.set('saleId', saleId)
  if (q) csvParams.set('q', q)
  if (state) csvParams.set('state', state)
  if (payment) csvParams.set('payment', payment)
  ...
  if (size) csvParams.set('size', size)
  const csvHref = `/api/admin/orders.csv${csvParams.size ? `?${csvParams.toString()}` : ''}`
  return (
    <>
      <div className="admin-heading"><div><span className="eyebrow">ORDERS</span><h1>주문 관리</h1><p>주문 검색, 입금 확인, 배송과 픽업 상태를 처리합니다.</p></div><a className="button button--secondary" href={csvHref}><Download size={15} /> CSV 다운로드</a></div>
      <form className="admin-filters admin-filters--extended"><select name="saleId" defaultValue={saleId} aria-label="판매 차수"><option value="all">전체 차수</option>{(sales ?? []).map((sale) => <option key={sale.id} value={sale.id}>{sale.sale_kind === 'test' ? '[테스트] ' : ''}{sale.round_number}차 · {sale.title}</option>)}</select><label><Search size={15} /><input name="q" defaultValue={q} placeholder="주문번호, 이름, 전화, 이메일, 입금자명" /></label><select name="state" defaultValue={state} aria-label="주문 상태"><option value="">모든 주문 상태</option>{Object.entries(orderStateLabel).map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select><select name="payment" defaultValue={payment} aria-label="입금 상태"><option value="">모든 입금 상태</option>{Object.entries(paymentStateLabel).map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select><select name="itemType" defaultValue={itemType} aria-label="상품"><option value="">모든 상품</option><option value="shirt">티셔츠</option><option value="bag">가방</option></select><select name="fulfillment" defaultValue={fulfillment} aria-label="수령 방법"><option value="">모든 수령 방법</option><option value="shipping">택배</option><option value="pickup">픽업</option></select><select name="overdue" defaultValue={overdue} aria-label="입금 기한"><option value="">모든 입금 시간</option><option value="1">1시간 경과</option></select><select name="size" defaultValue={size} aria-label="사이즈"><option value="">모든 사이즈</option>{sizes.map((value) => <option key={value}>{value}</option>)}</select><input name="from" type="date" defaultValue={from} aria-label="주문 시작일" /><input name="to" type="date" defaultValue={to} aria-label="주문 종료일" /><button className="button button--primary" type="submit">검색</button></form>
      <section className="admin-panel"><div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>차수</th><th>주문번호</th><th>주문자 / 입금자</th><th>상품</th><th>금액</th><th>주문 상태</th><th>입금 상태</th><th>접수일</th></tr></thead><tbody>{(orders ?? []).map((order) => { const isOverdue = order.order_state === 'payment_pending' && order.payment_state === 'pending' && Date.parse(order.payment_due_at) <= Date.now(); const sale = saleMap.get(order.sale_id); return <tr key={order.id}><td>{sale ? `${sale.round_number}차` : '-'}{sale?.sale_kind === 'test' && <small className="text-danger">테스트</small>}</td><td><Link href={`/admin/orders/${order.order_number}`}>{order.order_number}</Link>{isOverdue && <small className="text-danger">입금 1시간 경과</small>}</td><td>{order.customer_name}<small>{order.depositor_name}</small></td><td>{order.total_quantity}개</td><td>{order.total_amount.toLocaleString('ko-KR')}원</td><td><Badge tone={order.order_state === 'cancelled' ? 'red' : 'blue'}>{orderStateLabel[order.order_state as OrderView['orderState']]}</Badge></td><td>{paymentStateLabel[order.payment_state as OrderView['paymentState']]}</td><td>{new Date(order.created_at).toLocaleString('ko-KR')}</td></tr>})}</tbody></table>{!orders?.length && <div className="admin-empty">조건에 맞는 주문이 없습니다.</div>}</div></section>
    </>
  )
}
