    ...
    <>
      ...
      {!latest ? <section className="admin-panel"><p>복사할 기존 차수가 없습니다. 먼저 초기 데이터베이스 마이그레이션을 적용해주세요.</p></section> :
        <form className="admin-panel settings-form" action={createSaleAction}>
          <section><h2>복사 기준</h2><div className="form-grid"><div className="field field--full"><label>복사할 차수 <span className="required-mark">*</span></label><select name="sourceSaleId" defaultValue={latest.id} required>{(sales ?? []).map((sale) => <option key={sale.id} value={sale.id}>{sale.round_number}차 · {sale.title}</option>)}</select><span className="field__hint">주문과 고객 이미지는 복사하지 않습니다.</span></div></div></section>
          ...
          <div className="notice">새 차수는 고객에게 보이지 않는 <strong>초안</strong>으로 만들어집니다. 생성 후 전체 설정을 확인하고 공개해주세요.</div>
