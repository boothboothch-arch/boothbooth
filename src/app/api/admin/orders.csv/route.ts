...
export async function GET(request: NextRequest) {
  ...
  const rows = (orders ?? []).map((order) => {
    ...
    const itemSummary = orderItems.map((item) => { const selectedOptions = Array.isArray(item.selected_options) ? item.selected_options as { valueLabel?: string }[] : []; return `${item.product_name} / ${item.initial_text ? `${item.initial_text} (${item.initial_line_count ?? 1}줄)` : '이니셜없음'} / ${selectedOptions.map((option) => option.valueLabel).filter(Boolean).join('·') || '기본옵션'} / 스티커:${item.sticker_selected ? (item.sticker_categories ?? []).join('·') || '선택' : '미선택'} / 기타 요청:${item.extra_request || '-'}` }).join(' | ')
